...
# Library---------------------------------------------------
day_df = pd.read_csv("/mount/src/bike-share-wahyu-submission/Submission/dashboard/main_data.csv")
...
with st.sidebar:
    ...
    selected_dates = st.date_input(
        "Pilih rentang tanggal (dalam satu tahun yang sama)",
...
if len(selected_dates) == 2:
    ...
    # Validasi: Pastikan rentang tanggal dalam satu tahun yang sama
    if start_date.year != end_date.year:
        st.warning("Harap pilih rentang tanggal dalam satu tahun yang sama.")
        main_df = day_df  # Tampilkan semua data jika rentang tidak valid
    else:
        main_df = day_df[(day_df["dteday"] >= pd.to_datetime(start_date)) & 
                         (day_df["dteday"] <= pd.to_datetime(end_date))]
else:
    st.warning("Harap pilih rentang tanggal yang valid.")
    main_df = day_df  # Jika rentang tanggal tidak valid, tampilkan semua data
...
# Visualisasi 1---------------------------------------------
registered_df = main_df.groupby(by="mnth_name").registered.sum().reset_index()
registered_df.rename(columns={"registered": "user_count"}, inplace=True)
registered_df['user_type'] = 'Registered'

casual_df = main_df.groupby(by="mnth_name").casual.sum().reset_index()
...
# Pertanyaan 2------------------------------------------------
agg_df = main_df.groupby(by="mnth").agg({
    "temp": ["min", "max", "mean"],
    "hum": ["min", "max", "mean"],
